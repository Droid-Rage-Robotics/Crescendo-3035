...
public class ShooterToClaw extends SequentialCommandGroup {
    public ShooterToClaw(Shooter shooter, ClawElevator clawElevator, ClawPivot clawPivot, ClawIntake claw, Intake intake) {
        addRequirements();
        addCommands(
            ...
            claw.setTargetVelocityCommand(ClawIntake.Velocity.SHOOTER_TRANSFER),
            new WaitUntilCommand(()->claw.isElementInClaw()),//Wait Until it shows that it is in to continue
            claw.setTargetVelocityCommand(ClawIntake.Velocity.STOP),
