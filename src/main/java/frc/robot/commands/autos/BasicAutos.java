...
public final class BasicAutos {
    ...
    public static Command shootPLusTurnParkHuman(SwerveDrive drive, Intake intake, Shooter shooter, double wait) {
        return new SequentialCommandGroup(
            ...
            PathPlannerFollow.create(drive, "R1+F1Blue")
                ...
                .build()
            ,new WaitCommand(3),
            ShooterCommands.setIntakeAndShooter(intake, Intake.Value.CLIMB_DOWN, shooter, Shooter.ShooterSpeeds.HOLD)
        );
    }
    ...
    public static Command toCenter(SwerveDrive drive, Intake intake, Shooter shooter) {
        return new SequentialCommandGroup(    
            ShooterCommands.shootPreload(intake, Intake.Value.SHOOTER_TRANSFER, shooter, ShooterSpeeds.AUTO_SPEAKER_SHOOT),
            new WaitCommand(1),
            ShooterCommands.setIntakeAndShooter(intake, Intake.Value.INTAKE_GROUND, shooter, Shooter.ShooterSpeeds.STOP),
            new WaitCommand(1),
